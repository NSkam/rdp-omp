...
void RamerDouglasPeucker(const vector<Point> &pointList, double epsilon, vector<Point> &out)
{
	...
	if (dmax > epsilon) {
		...
		#pragma omp task firstprivate(firstLine) shared(recResults1)
		{
		RamerDouglasPeucker(firstLine, epsilon, recResults1);
			
		}
		#pragma omp task firstprivate(lastLine) shared(recResults2)
		{
...
int main(int argc, char *argv[])
{
	...
	#pragma omp parallel shared(AllPolylines,SimplifiedAllPolylines) private(wait_time) 
	{wait_time = omp_get_wtime();

	#pragma omp for ordered schedule(dynamic,7) private(polyline,simplified_polyline,line)  
	...
	wait_time = omp_get_wtime() - wait_time;


	printf("Time for thread %d calculation = %f sec\n",omp_get_thread_num(), wait_time);
	}
