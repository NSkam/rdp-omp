...
int main(int argc, char *argv[])
{
	...
	struct timeval			thread_time_start, thread_time_end;
	int total_time_thread;
	...
	gettimeofday(&thread_time_start, NULL);

	#pragma omp parallel private(polyline,simplified_polyline,line) shared(AllPolylines,SimplifiedAllPolylines)
	{gettimeofday(thread_time_start, NULL);
	#pragma omp parallel for schedule(static,1000000)  //to chunk epeireazei ta apotelesmata ws twra >7 bgazei swsta me polla threads
	...
	gettimeofday(&thread_time_end, NULL);
	total_time_thread = ((thread_time_end.tv_sec - thread_time_start.tv_sec) * 1000000.0 + (thread_time_end.tv_usec - thread_time_start.tv_usec));
	
	cout << "\n The thread with number" << omp_get_thread_num() << "has enden in" << total_time_thread / 1000000.0 	<< "\n";
